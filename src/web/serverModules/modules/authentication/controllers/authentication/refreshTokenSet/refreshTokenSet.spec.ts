        ...
        describe('authentication', () => {
          describe('refresh token controller', () => {
            ...
            describe('Error path - service', () => {
              ...
              it(`Should validate request query parameters`, () => {
                expect(spiedQueryValidator).toHaveBeenCalledTimes(1);
                expect(spiedQueryValidator).toHaveBeenCalledWith(REQUEST.query);
              });

              it(`Should call 'authenticationService.getTokensSet', after query has been validated`, () => {
                expect(authenticationService.refreshTokens).toHaveBeenCalledTimes(1);
                expect(authenticationService.refreshTokens).toHaveBeenCalledWith(REQUEST.query);
                expect(spiedQueryValidator).toHaveBeenCalledBefore(authenticationService.refreshTokens);
              });
                ...
                tokenSetOrError
                  .do({
                    ...
                    left: (error: AppError) => {
                      ...
                      expect(error).toHaveProperty('message', 'Validation failed: ["Missing mandatory query parameter: refresh_token"]');
