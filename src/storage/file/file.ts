...
import { FSReadOptions, FSPath, FSWriteOptions, FSMKDirOptions, FileStorage, FSReadTextOptions } from './file.types';
...
const fileStorage: FileStorage = {
  readFileString: async (path: FSPath, encoding: FSReadTextOptions = DEFAUTL_ENCODING): Promise<Either<AppError, string>> =>
    readFileString(path, encoding),

  readFileBuffer: async (path: FSPath, options?: FSReadOptions): Promise<Either<AppError, Buffer>> =>
    readFile(path, options),

  writeFile: async (path: FSPath, data: any, options: FSWriteOptions): Promise<Either<AppError, void>> =>
